...
class FitB:
    def __init__(self, model, topic, grade_level, prompt_fill_in_blank, parser_fill_in_blank, verbose):
        self.model = model
        self.topic = topic
        self.grade_level = grade_level
        self.prompt_fill_in_blank = prompt_fill_in_blank
    ...
    def create_fill_in_blank(self, num_fill_in_blank) -> str:
        prompt = PromptTemplate(
            template=self.prompt_fill_in_blank,
            input_variables=["topic", "grade_level"],
            partial_variables={"format_instructions": self.parser_fill_in_blank.get_format_instructions()}
        )

        chain = prompt | self.model | self.parser_fill_in_blank

        if self.verbose: logger.info(f"Chain compilation complete")

        attempts = 0
        max_attempts = num_fill_in_blank * 5
        question_bank = set()
        generated_questions = []
        ...
        if len(generated_questions) < num_fill_in_blank:
            logger.warning(f"Only generated {len(generated_questions)} out of {num_fill_in_blank} requested questions")
        
        generated_questions = generated_questions[:num_fill_in_blank]
        generated_questions_string = 'Fill in the blank Questions\n'
        for i, gq in enumerate(generated_questions):
            generated_questions_string += 'FitB ' + str(i + 1) + ':\n'
            fitb_string = self.fitb_to_string(gq)
            generated_questions_string += fitb_string + '\n'
...
class WorksheetBuilder:
    def __init__(self, topic, grade_level, prompt_summary=None, prompt_multiple_choice=None, prompt_fill_in_blank=None, model=None, parser_multiple_choice=None, parser_fill_in_blank=None, prompt_open_ended=None, parser_open_ended=None, verbose=False):
    ...
    def create_worksheets(self, num_worksheets: int = 1, num_multiple_choice: int = 1, num_fill_in_blank: int=1, num_open_ended: int=1) -> List[str]:
        ...
        fitb = FitB(self.model, self.topic, self.grade_level, self.prompt_fill_in_blank, self.parser_fill_in_blank, self.verbose)
        for i in range(num_worksheets):
