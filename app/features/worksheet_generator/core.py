...
def executor(grade_level : int, topic: str, question_types: list[ToolQuestionType], verbose=False):
    
    try:
        if verbose: logger.debug(f"grade_level: {grade_level}, topic: {topic}, question_types: {question_types}")
        
        params = {"grade_level": grade_level, "topic": topic, "question_types": question_types, "verbose": verbose}
