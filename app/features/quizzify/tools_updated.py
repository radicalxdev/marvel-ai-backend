...
class RAGpipeline:
    ...
    def load_PDFs(self, files) -> List[Document]:
        ...
        if isinstance(files, ToolFile):
            total_loaded_files = self.loader.load(files)
        elif isinstance(files, str):
            webpage_loader = WebPageLoader(files, self.verbose)
            total_loaded_files = webpage_loader.load()
        else:
            raise ValueError("Unsupported input type. Expecting ToolFile or URL.")
